...
int main(int argc, char **argv)
{
  ...
  Mz = atoi(argv[6]);    // note I add Mz on Nov 27, 2011;
   ...
   Npmlx = atoi(argv[7+1]);
    ...
    optmax = atoi(argv[31+1]);
    sparedouble = atof(argv[32+1]);
     RRT = atof(argv[33+1]);
   */
  ...
  int optmax = atoi(argv[32]);
  double sparedouble = atof(argv[33]); //use it to store lcenter;
...
PetscPrintf(PETSC_COMM_WORLD,"RRT given is %.12e \n",RRT);
 ...
 int numofvar=1;
  ...
  myfundatatypeq myfundata = {Nx, Ny, Nz, hx, hy, hz, omega, sparedouble, spareptdouble, ksp, epspmlQ, epscoef, epsC, epsCi, epsP, x, cglambda, b, weightedJ, vR, epsSReal, epsgrad, vgrad, vgradlocal, tmp, tmpa, tmpb, A, D, M, from, to, scatter, filenameComm};
